...
class ApprovedChangeTests(unittest.TestCase):
    ...
    def render(self, path, query):
        ...
        with template_rendered.connected_to(capture, self.app):
            response = self.client.get(path, query_string={**query, 'run': '1'})
    ...
    def test_benchmark_invalid_sort_bypasses_query(self):
        for args in ({"sort": "not-a-column"}, {"direction": "DROP TABLE Country"}):
            with patch("immunisation_app.views.get_above_global_infections") as query:
                response = self.client.get("/infection-benchmark", query_string={**args, 'run': '1'})
                self.assertIn(b'role="alert"', response.data)
                query.assert_not_called()

    def test_improvement_selects_biggest_n_before_every_presentation_sort(self):
        self.db.execute("DELETE FROM Vaccination WHERE antigen='MCV1' AND year IN (2000,2024)")
        for country, end in (("AFG", 20), ("ALB", 40), ("AGO", 30), ("DZA", 50)):
            for year, doses in ((2000, 10), (2024, end)):
                self.db.execute("UPDATE CountryPopulation SET population=100 WHERE country=? AND year=?", (country, year))
                self.db.execute("INSERT INTO Vaccination (inf_type,antigen,country,year,doses) VALUES ('MEA','MCV1',?,?,?)", (country, year, doses))
        for column in ("country", "start_rate", "end_rate", "improvement"):
            for direction in ("asc", "desc"):
                with self.subTest(column=column, direction=direction):
                    rows = get_vaccination_improvements(
                        self.db, antigen="MCV1", start_year=2000, end_year=2024,
                        limit=3, sort_by=column, direction=direction,
                    )
                    self.assertEqual({r["country_id"] for r in rows}, {"DZA", "ALB", "AGO"})
                    self.assertEqual({r["country_id"]: r["improvement"] for r in rows},
                                     {"DZA": 40.0, "ALB": 30.0, "AGO": 20.0})
                    values = [r[column] for r in rows]
                    self.assertEqual(values, sorted(values, reverse=direction == "desc"))

    def test_improvement_route_explains_fixed_top_n(self):
        memberships = []
        for column in ("country", "start_rate", "end_rate", "improvement"):
            html, ctx = self.render("/vaccination-improvement", {
                "antigen": "MCV1", "start_year": 2000, "end_year": 2024,
                "limit": 3, "sort": column, "direction": "asc",
            })
            self.assertIn("largest improvements are selected first", html)
            memberships.append({r["country_id"] for r in ctx["rows"]})
        self.assertTrue(all(s == memberships[0] for s in memberships))
    ...
    def test_invalid_numeric_inputs_bypass_query(self):
        ...
        for args in invalid:
            with self.subTest(args=args), patch("immunisation_app.views.get_infection_by_economy") as query:
                response = self.client.get("/infections", query_string={**args, 'run': '1'})
                self.assertIn(b'role="alert"', response.data)
                query.assert_not_called()
